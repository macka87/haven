...
//
//  
//	Model:	CY7C1513AV18 4M X 18 QDR(tm)-II Burst-of-4 SRAM
// 	Date:		March 9, 2007
//
//	Revision:	1.0
//
...
`define no_words (1048576 -1)	//1M x 18 RAM (uses 4)
...
`define tkh	#1.32
`define tkl	#1.32
`define tkhkh	#1.49				////  CHECK
`define tkhch	#(0.0:0.9:1.8)			////  CHECK
`define tco	#0.25
`define tdoh	#0.25
`define tccqo   #0.45
`define tcqoh   #0.45	
...
begin
  	if ($realtime > 0) 
	   $display("NOTICE      : 020 : BWS0 problem on CLK_n");
end

always @(noti2_2)
begin
  	if ($realtime > 0) 
	   $display("NOTICE      : 020 : BWS1 problem on CLK");
end
...
begin
	...
	begin
//		if (rpen_o_o_o == 0)		/// takes into account if there  is skew b/w K & C
		if (rpen_o_o == 0)
		   begin
			Data_out =  `tco mem1[Read_Address_o];
		//				$display("burst 1", $time);
...
begin
//	if (rpen_o_o == 0 || rpen_o_o_o == 0) // commented july 21
	if (rpen_o_o == 0 || rpen_o_o_o == 0 || rpen_o_o_o_o == 0)
		tristate = `tchz 0;
	else
		tristate = `tchz 1;
...
begin 
	state = `reset;
	prev_state = `reset;
	vendor_code = 32'b00111010011011010100000001101001;	// IDCODE for QDR2 CY7C1513AV18 device. This may not be changed.	
	tdo_tristate = 1;
	extest_en = 0;
	samplz_en = 0;
	extest_oe = 0;
